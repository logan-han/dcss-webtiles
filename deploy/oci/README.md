...
3. DNS: `deploy/oci/dns.sh <public-ip>` swaps the current CNAME for an A record (needs the local AWS login). Do it before the first start, or restart Caddy
   afterwards so it can pass the certificate challenge:
